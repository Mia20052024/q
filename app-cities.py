...
capital_filter = st.sidebar.multiselect( 'Capital Selector', df.capital.unique(), # options
...
form. form_submit_button("Apply")# filter by population
df = df[df-population >= population_filter]# filter by capita]
df = df [df. capital.isin(capital_filter)]
if country_filter!='ALL': df = df [df.country == country_filter]
st.map(df)
st.subheader('city details')
st.write(df[['city', 'country', 'population', 'capital']])
